...
```json
{
  ...
  "vision": {"mode": "range", "range": 3}   // 視野制限（下記参照）
}
```

#### 視野制限 (Vision Restriction)

視野制限ギミックは、プレイヤーの視界を制限し、マップの一部のみを表示します。

**パラメータ:**
- `mode`: 視野制限のモード
  - `"range"`: 周囲kマスの範囲のみ見える（チェビシェフ距離）
  - `"line_of_sight"`: 視線が壁にぶつかるまでの範囲が見える（レイキャスティング）
- `range`: `mode="range"` の場合の視野範囲（デフォルト: 3）
...
- 各層によって異なるギミックがある（同一のギミックがあることもある）。
  ...
  - **視野制限**: プレイヤーの視界を制限し、周囲のk マスのみ見える、または視線が壁にぶつかるまでの範囲のみ見える。
